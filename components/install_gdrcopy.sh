...
if [[ "$TARGET_NODE_TYPE" == "azure_vm_akshost" ]]; then 
    ...
    dpkg_install_with_lock_wait "gdrdrv-dkms_${GDRCOPY_VERSION}_arm64.${GDRCOPY_DISTRIBUTION}.deb"
    ...
    if [[ $DISTRIBUTION == "azurelinux3.0" ]]; then
        ...
        if [[ $DISTRIBUTION == *"ubuntu"* ]]; then
            ...
            dpkg_install_with_lock_wait "gdrdrv-dkms_${GDRCOPY_VERSION}_${ARCHITECTURE_DISTRO}.${GDRCOPY_DISTRIBUTION}.deb"
            apt-mark hold gdrdrv-dkms
            dpkg_install_with_lock_wait "libgdrapi_${GDRCOPY_VERSION}_${ARCHITECTURE_DISTRO}.${GDRCOPY_DISTRIBUTION}.deb"
            apt-mark hold libgdrapi
            dpkg_install_with_lock_wait "gdrcopy-tests_${GDRCOPY_VERSION}_${ARCHITECTURE_DISTRO}.${GDRCOPY_DISTRIBUTION}+cuda${CUDA_DRIVER_VERSION}.deb"
            apt-mark hold gdrcopy-tests
            dpkg_install_with_lock_wait "gdrcopy_${GDRCOPY_VERSION}_${ARCHITECTURE_DISTRO}.${GDRCOPY_DISTRIBUTION}.deb"
