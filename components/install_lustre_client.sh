...
RPM_MACROS
    ...
    #
    # Why install BOTH static kmod-lustre-client AND lustre-client-dkms:
    # kmod-lustre-client-tests has rpm-level ksym() Requires that pin
    # specific Module.symvers hashes of every kernel symbol it imports
    # from the core lustre kmod (e.g.
    # `Requires: ksym(__cfs_fail_check_set) = 0xcf61bacb`). Those
    # ksym() Provides are only generated by RHEL's __find_provides when
    # processing a real .ko file, which only the static kmod-lustre-client
    # ships. lustre-client-dkms ships source and builds the .ko at
    # postinstall time, so its rpm-level metadata only carries the
    # package-level `Provides: kmod-lustre-client = %{version}` -- enough
    # to satisfy lustre-client's `Requires: kmod-lustre-client` but NOT
    # kmod-lustre-client-tests's ksym() Requires.
    #
    # Installing both is fine: the static kmod owns the .ko files for
    # the bake kernel (matching what kmod-lustre-client-tests was linked
    # against), and `dkms autoinstall` for the bake kernel is idempotent
    # (it detects the existing .ko files; in the worst case it backs
    # them up and reinstalls bit-identical replacements from the same
    # source). On future kernel upgrades, the static kmod becomes
    # inactive for the new kernel and lustre-client-dkms rebuilds the
    # core module; kmod-lustre-client-tests's static .ko files remain
    # bake-kernel-only (already an accepted limitation -- see
    # repo/lustre-tests-marker-rpm.md).
    dnf install -y ./lustre-client-[0-9]*.$(uname -m).rpm \
                   ...
                   ./kmod-lustre-client-[0-9]*.$(uname -m).rpm \
