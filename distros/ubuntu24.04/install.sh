...
if [[ "$GPU" == "MAIA" ]]; then
    ...
    # GRUB DMA reservation, automatic-upgrade lockdown, kernel header
    # alignment, driver load and device node services, then the Trivy scan.
    # Nothing after this branch applies to MAIA, hence the exit 0.
    $COMPONENT_DIR/maia_tuning.sh
