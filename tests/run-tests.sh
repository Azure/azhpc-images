...
then
    ...
    CHECK_NCCL=1
    CHECK_GCC=0
elif [[ $distro == "SUSE Linux Enterprise High Performance Computing 15 SP3" ]]
then
    . ../suse/sle-15-spx-hpc/sle-15-sp3-hpc/set_environmental_variables.sh
    # HPCX_OMB_PATH=${HPCX_OMB_PATH_SLE_HPC}
    CHECK_GCC=0
    CHECK_ONEAPI=0
    CHECK_AZURE_HPC_DIAG=0    
    CHECK_HPCX=1
    CHECK_IMPI_2021=0
    CHECK_MVAPICH2=0
    CHECK_OMPI=0
    CHECK_BLIS_MT=0
    # MODULE_FILES_ROOT=${MODULE_FILES_ROOT_SLE_HPC}
    # MOFED_VERSION=${MOFED_VERSION_SLE_HPC}
    # IMPI2021_PATH=${IMPI2021_PATH_SLE_HPC}
    # MVAPICH2_PATH=${MVAPICH2_PATH_SLE_HPC}
    # MVAPICH2X_PATH=${MVAPICH2X_PATH_SLE_HPC}
    # OPENMPI_PATH=${OPENMPI_PATH_SLE_HPC}
    CHECK_AOCL=0
    CHECK_NV_PMEM=0
    CHECK_NCCL=0
else
...
# verify Intel oneAPI Math Kernel Library
